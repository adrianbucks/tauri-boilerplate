...
│   ├── index.ts
│   ├── engine/
│   │   ├── AuthorizationEngine.ts    ← can() / require() implementation
│   │   ├── PermissionResolver.ts     ← Resolves effective permissions from roles + scopes
│   │   └── ScopeEvaluator.ts         ← Evaluates resource-dimension constraints
│   ├── types.ts
│   └── errors.ts
```
