...
Android lifecycle: the iroh node must handle foreground/background/suspended/terminated transitions. **RESEARCH REQUIRED** — see R-006 in doc 17 — confirm what background networking Android reliably permits before promising continuous background sync.
...
Two paths — decided by iroh-docs Spike 0.5. The platform package boundary is the same for both paths.
...
**Proceed with this option only if Spike 0.5 confirms**:
...
**RESEARCH REQUIRED** — see R-001 in doc 17: confirm HLC representation before implementing.
...
HLC format (design decision during Spike 0.3):
