...
│   ├── index.ts
│   ├── device/
│   │   ├── DeviceIdentity.ts        ← Interface + Tauri bridge
│   │   └── DeviceInitialisation.ts  ← First-launch key generation flow
│   ├── user/
│   │   ├── UserIdentity.ts          ← Interface
│   │   └── UserSession.ts           ← Session creation, validation
│   └── types.ts                     ← Shared identity types
```
...
The `SecureStorage` trait is the abstraction point. Platform selection is resolved during Spike 0.6.

---

## Secure key storage research (RESEARCH REQUIRED)

**See**: Research item R-004 in doc 17, and ADR-019.
...
- Evaluated in Spike 0.6 with ADR written
...
**RESEARCH REQUIRED** — see doc 17, R-004:
