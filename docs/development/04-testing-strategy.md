...
tests/sync/                   ← Multi-device sync harness tests
tests/security/               ← Security regression suite
tests/e2e/                    ← End-to-end Tauri application tests
```
...
Run manually (not in CI by default — too hardware-dependent).
...
End-to-end tests drive the actual Tauri application using a test framework.
...
| Pipeline job          | Trigger            | Command                       |
| --------------------- | ------------------ | ----------------------------- |
| Lint                  | Push to any branch | `pnpm turbo lint`             |
| Typecheck             | Push to any branch | `pnpm turbo typecheck`        |
| Unit tests            | Push to any branch | `pnpm turbo test`             |
| Rust tests            | Push to any branch | `cargo test --workspace`      |
| Integration tests     | Push to main/PR    | `pnpm test:integration`       |
| Security tests        | Push to main/PR    | `pnpm test:security`          |
| Sync tests            | Push to main/PR    | `pnpm test:sync`              |
| E2E tests             | PR / release tag   | See E2E job                   |
| Performance baselines | Release tags only  | Manual + automated comparison |
