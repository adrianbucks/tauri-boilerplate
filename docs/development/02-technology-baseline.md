...
| Local database       | SQLite                               | Bundled with Tauri                                            | —                                          |
...
| P2P transport        | iroh                                 | Latest stable                                                 | https://github.com/n0-computer/iroh        |
...
## Research-required (Phase 0 decision gates)

These technologies are **candidate choices** until Phase 0 spikes confirm compatibility. Do not build platform abstractions around them until the ADR is written.

### Critical path (block Phase 1)
...
| iroh-docs                                     | Can it replace a custom operation log? Answers 15 specific questions — see doc 09 | ADR-013          |
...
**RESEARCH REQUIRED** — Evaluate before implementing `packages/core`.
