...
Work proceeds in six phases. No phase begins before the previous phase's critical deliverables are complete.

**Critical constraint**: Phase 0 spikes must be finished before any production platform code is stabilised. The Phase 0 research answers decide the exact implementation of sync, identity, and the database access path.
...
| -------------------------- | ----- | ------------ | ------------------------------------------------------------------------------ |
| M1 — Bootable Platform     | 1     | ✅ COMPLETED | SQLite + Drizzle + migrations + feature registry running, Turbo CI passing     |
| M2 — Secure Local Platform | 2     | ✅ COMPLETED | Identity, RBAC, audit, sync groups, UI (Tailwind v4 + shadcn), demo app booted |
| M3 — P2P Proof             | 3     | ✅ COMPLETED | HandshakeProtocol & PairingService under 7-layer sync authorization control    |
| M4 — Replication Decision  | 4     | ✅ COMPLETED | HybridLogicalClock, conflict policies, multi-device sync replication harness   |
| M5 — Complete Platform     | 5     | ✅ COMPLETED | Import/export (SheetJS), hardware barcode scanning, vertical-slice demo app    |
| M6 — Release Pipeline      | 6     | ✅ COMPLETED | GitHub Actions CI/CD workflows for Windows (.msi/.exe) & Android (.apk)        |
...
**Rule**: Phase 0 produces only spike code (in `apps/platform-spike/`, deleted afterwards) and ADR documents. No spike code is promoted to a platform package.
...
Prove on **real physical hardware** (not emulator):

- [ ] Windows `.msi` installer builds and launches
- [ ] Android APK builds and launches on a physical device (API 35+)
- [ ] React/Vite frontend loads inside Tauri webview
- [ ] TypeScript Tauri command call returns a Rust value
- [ ] Rust returns a structured `PlatformError` (not raw string)
- [ ] Capability scoping: filesystem read capability granted narrowly, not globally
- [ ] Hot reload works in dev mode (`tauri dev`)
- [ ] Production bundle sizes are acceptable
- [ ] Updater signing key generation works (document the key management process)
...
```
- [ ] ADR-001 through ADR-014 written
- [ ] ADR-019 written (secure storage)
- [ ] apps/platform-spike/ deleted
- [ ] All spike acceptance criteria met
- [ ] iroh-docs decision recorded
- [ ] SQLite integration path recorded
- [ ] Secure storage path recorded
```
...
- Full core platform schema (see doc 06)
...
- Minimum event catalogue (full list in doc 16)
...
- Full `SyncState` machine (13 states — see doc 09)
...
The complete Locations & Warehouses scenario (see doc 13).

### 5.6 — Full test suite

All sync harness, security regression, property-based, and performance baseline tests.

**Milestone 5 acceptance**: All items in the platform Definition of Done (doc 14) pass.
