...
| | |
| **Current** | Package boundaries, typed errors, repositories, tombstone fields, and RBAC concepts exist. Demo writes are not yet atomic business-plus-audit-plus-replication transactions. |
| **Target** | Four independent layers (data, identity, authorization, sync) and a seven-layer pre-sync stack that never treats reachability as authorization. |
| **Remaining** | Enforce these rules end to end, including real transport admission. See [status](../verification/status.md). |

Treat every rule in this document as a non-negotiable coding standard for new work. Treat it as an acceptance criterion, not as evidence that the current codebase already satisfies it.
...
18. **Offline/online availability is explicitly documented per action type.** (See [architecture/06](./06-synchronisation-and-p2p.md).)
...
The package boundaries, typed error model, correlation IDs, repository abstractions, tombstone fields, and RBAC concepts exist. The seven-layer sync stack is not yet enforced end to end: `SyncManager` and `PairingService` implement partial checks, not real transport admission. Demo feature writes also do not yet provide the documented atomic business-plus-audit-plus-replication transaction.

Keep these principles as non-negotiable future rules. Treat them as acceptance criteria, not as evidence that the current implementation already satisfies them.
