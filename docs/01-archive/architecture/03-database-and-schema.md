...
| | |
| **Current** | `DatabaseConnection` plus `MemoryDatabaseConnection` (`sql.js`), `BaseRepository`, and an owner-aware TypeScript `MigrationEngine` with checksums. Demo bootstrap still creates an in-memory database and ad hoc tables. |
| **Target** | One durable SQLite file per app instance; UI → service → repository → database package → native SQLite; migrations as a startup readiness gate. |
| **Remaining** | Native adapter, restart/WAL/backup tests, platform migration set. [S-01](../specifications/S-01-database-and-persistence.md), [R-02](../research/R-02-sqlite-persistence-and-migrations.md). |

Patterns below are the persistence standard. File trees that do not match `packages/database/src` today are target layout, not a live inventory.
...
```
packages/database/src/          ← current
├── index.ts
├── connection/
│   ├── DatabaseConnection.ts
│   └── MemoryDatabaseConnection.ts   ← sql.js test/demo adapter
├── migrations/
│   └── MigrationEngine.ts
├── repository/
│   └── BaseRepository.ts
...
Target additions (not present): DatabaseHealth, FeatureMigrationEngine,
TransactionRunner as a separate type, and a native SQLite adapter.
```
...
ADR-003 and ADR-004 record the intended access path. The production native SQLite binding is still a research gate ([R-02](../research/R-02-sqlite-persistence-and-migrations.md), [R-03](../research/R-03-rust-architecture-and-api-integrity.md)). Today TypeScript talks to `sql.js` through `MemoryDatabaseConnection`.

### Preferred path (native production target)
...
The delete operation generates a `delete` sync operation that propagates the tombstone to peers. See [architecture/06](./06-synchronisation-and-p2p.md) for the target tombstone lifecycle and garbage collection.

---

## Backup strategy

**RESEARCH REQUIRED** — see [R-02](../research/R-02-sqlite-persistence-and-migrations.md) and ADR-003.
...
`packages/database` currently provides a `DatabaseConnection` abstraction, a `MemoryDatabaseConnection` backed by `sql.js`, repositories, soft-delete mechanics, transactional savepoints, and an owner-aware TypeScript migration engine with checksum validation. The demo still creates an in-memory database and ad hoc tables during bootstrap. There is no production Rust/sqlx SQLite adapter, durable restart path, native backup path, or complete platform migration set yet.

The migration tests are useful boundary tests, but they do not prove durable native persistence. Future database work must preserve append-only migrations, `(owner, version)` identity, checksum failure, transaction rollback, foreign-key enforcement, explicit pragma verification, and repository-only feature access.
