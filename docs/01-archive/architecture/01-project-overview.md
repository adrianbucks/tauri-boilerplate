...
| | |
| **Current** | GitHub template scaffold with TypeScript platform packages, Rust crates, demo app, CI, and unsigned local Windows/Android packaging. |
| **Target** | Opinionated local-first platform: durable SQLite, cryptographic identity, scoped RBAC, iroh sync, and a replaceable application shell. |
| **Remaining** | Production gates in [S-01](../specifications/S-01-database-and-persistence.md)–[S-12](../specifications/S-12-release-and-distribution.md). Status: [verification/status.md](../verification/status.md). |

The sections below describe the intended product and downstream usage model. They are the design to implement toward, not a claim that every capability already ships.
...
The boilerplate is designed as an opinionated platform: identity, authorisation, data synchronisation, audit, import/export, hardware abstraction, and a UI component set. Developers building business applications start here and add only their domain-specific features. Several of those platform paths are still incomplete; see [Implementation Status](../verification/status.md).

The architecture is broader than the current implementation. Several production paths remain incomplete, including durable native SQLite, secure key storage, real iroh transport, durable operation replication, recovery, signing, and complete native command governance. See [Implementation Status](../verification/status.md) for the evidence-backed position.
...
The repository currently provides the monorepo boundaries, TypeScript platform packages, Rust workspace, feature registry, demo application, CI checks, and verified local Windows/Android packaging. It is still a development scaffold rather than a production platform. Durable native SQLite, real secure identity, real iroh transport, durable replication, signing, and complete native capability governance remain incomplete. Use the [status matrix](../verification/status.md) and [roadmap](../development/roadmap.md) as the current source of truth.
