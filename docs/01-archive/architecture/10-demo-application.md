...
| | |
| **Current** | Showcase with dashboard, diagnostics, organisation, admin, and widget pages. Registers example, organisations, and identity-admin features. Unsigned local MSI/NSIS and APK builds. |
| **Target** | Vertical slice that proves pairing, multi-device sync, first-launch identity, import/export, and scanning on real hardware, while remaining a replaceable consumer of public APIs. |
| **Remaining** | Warehouse/location scenario, real pairing/sync, second-app proof. [S-09](../specifications/S-09-frontend-and-application-boundary.md). |

The warehouse domain below is a future acceptance scenario, not the current UI.
...
3. **Release artefact** — produces installable Windows and Android binaries that demonstrate the platform (unsigned until S-12 signing is configured)
...
The current demo is a platform showcase with dashboard, diagnostics, organisation, admin, and widget pages. It registers the example, organisations, and identity-admin features and consumes bootstrapped services through the platform context. It does not implement the warehouse/location scenario described above, real device pairing, real multi-device sync, first-launch identity setup, Android camera scanning, diagnostic bundle export, or signed release artifacts. The local Windows MSI/NSIS and Android APK builds are verified, but they are unsigned.

Use the documented warehouse/location scenario as a future acceptance test, not as a description of the current application. Keep demo-only domain behavior out of reusable platform packages and make the demo consume the same public interfaces a downstream application would use.
