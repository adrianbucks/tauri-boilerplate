# 09 — Synchronisation and P2P

| | |
| **Current** | Operation types, namespaces, handshake metadata validation, HLC, conflict policy types, state machine, pairing checks, and an in-process test harness. No iroh crate; transport manager is a stub; `SyncManager` records simulated metadata. |
| **Target** | iroh transport, seven-layer admission before any payload, durable operation log, idempotent apply, queues, quarantine, tombstone replication. |
| **Remaining** | [S-05](../specifications/S-05-sync-operation-log.md), [S-06](../specifications/S-06-iroh-transport-and-handshake.md), [S-07](../specifications/S-07-conflict-tombstone-and-recovery.md), [R-06](../research/R-06-iroh-transport-and-iroh-docs.md), [R-07](../research/R-07-local-first-replication-and-conflicts.md). |

iroh lifecycle and replication options below are the design to implement after research spikes. They are not a description of running P2P.
...
Android lifecycle: the iroh node must handle foreground/background/suspended/terminated transitions. **RESEARCH REQUIRED** — see [R-06](../research/R-06-iroh-transport-and-iroh-docs.md) — confirm what background networking Android reliably permits before promising continuous background sync.
...
## Replication architecture decision (open research gate)

Two paths — still decided by a current iroh-docs spike ([R-06](../research/R-06-iroh-transport-and-iroh-docs.md), [ADR-013](../decisions/ADR-013-iroh-docs-evaluation.md)). The platform package boundary is the same for both paths.
...
**RESEARCH REQUIRED** — see [R-07](../research/R-07-local-first-replication-and-conflicts.md): confirm HLC representation before implementing.
...
The repository currently provides sync operation types, namespace generation, handshake metadata validation, HLC logic, conflict policy types, sync state transitions, and an in-process test harness. There is no iroh dependency or real peer transport; the Rust transport manager is a placeholder, and `SyncManager` records simulated metadata rather than transmitting, applying, deduplicating, retrying, or quarantining durable operations. The demo sync tests therefore do not prove multi-device P2P behavior.

The operation ID, HLC, namespace, quarantine, tombstone, and seven-layer admission rules remain the target contract. Implement them only after the active iroh and replication research items produce a current spike and decision.
