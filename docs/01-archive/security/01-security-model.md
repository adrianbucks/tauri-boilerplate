...
| | |
| **Current** | Capability file for the demo, one identity Tauri command, RBAC/pairing security tests. Seven-layer admission is not enforced on real transport. |
| **Target** | Sync never becomes the security layer; every command is contracted; private keys never leave Rust; revocation propagates to connected and reconnecting peers. |
| **Remaining** | [S-10](../specifications/S-10-tauri-native-capability-boundary.md), [S-06](../specifications/S-06-iroh-transport-and-handshake.md), [R-09](../research/R-09-testing-supply-chain-and-release-integrity.md). |

Capability JSON examples below are the scoping pattern. They are not a complete live capability matrix.
...
Every device **must** generate an Ed25519 key pair at first launch (see [architecture/04](../architecture/04-identity-and-authentication.md)). The private key must be stored in OS-native secure storage — never in SQLite, never in JavaScript memory. The current `identity-core` crate does not do this.
...
**RESEARCH REQUIRED** — [R-05](../research/R-05-authentication-authorization-and-tenancy.md): design offline revocation and membership lease model.
...
1. Add or update a test in `tests/security/` (see [testing strategy](../development/04-testing-strategy.md))
...
- Dependabot or Renovate monitors dependency updates (see [build/release](../development/05-build-release-and-versioning.md))
...
**RESEARCH REQUIRED** — see [R-09](../research/R-09-testing-supply-chain-and-release-integrity.md): finalise the supply chain security toolchain choice. Checksums, pinned GHA SHAs, and audit jobs are target controls; do not assume they all run in CI today.

## Current implementation status

RBAC and pairing/sync-authorization tests exist. Private keys are not generated or stored. Command authorization beyond `get_device_identity` is unspecified because those commands do not exist. Treat this document as the threat model and coding standard for security work.
