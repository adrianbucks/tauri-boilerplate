...
export class WidgetRepository extends BaseRepository<WidgetRecord> {
  ...
  async findBySku(
    ...
    const sql = `SELECT * FROM widgets WHERE sku = ? AND deleted_at IS NULL LIMIT 1`;
    const rows = await executor.query<WidgetRecord>(sql, [sku]);
    return rows[0] ?? null;
  }
  ...
  async findBySyncGroup(
    ...
    const executor = this.getExecutor(tx);
    const sql = `SELECT * FROM widgets WHERE sync_group_id = ? AND deleted_at IS NULL ORDER BY name ASC`;
    return executor.query<WidgetRecord>(sql, [syncGroupId]);
  }
}
